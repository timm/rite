...
- Two facets in the workdir flags.py: topic (which literature) and technology (how the method works). SSOT; edit nowhere else.
- Never code from title+abstract: measured here, abstracts miss about half the flags (lit/coding-full.md, n=22).
- Binary matching over full text saturates; use the per-1k thr threshold (engine etc/recode.py).
...
engine etc/recode.py), the abstract must earn on its own. Same
